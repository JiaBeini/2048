...
<div >
  ...
  <div class="score-mul">
    ...
    <!-- 显示对方棋盘 -->
  <div class="otherGame" >
      <div class="otherMask" v-if="otherGameover">
      <p>对方游戏结束</p>
      <p>对手的分数是</p>
      <p>{{otherScore}}</p>
      </div>
    ...
    <div class="number-cells">
      ...
      <transition-group name="appear">
        ...
        top: ${getOtherTop(cell)};
        left: ${getOtherLeft(cell)};
        `
          "
        >
        <!-- cell的值 -->
  ...
  <!-- 这里是初始化棋盘按钮 -->
  <div class='show-mul'>
    <!-- <button @click="init" class="init-button-mul">新游戏</button> -->
  </div>
  ...
  <div class='container'>
    ...
    <div class="mask" v-if="gameover">
      <div v-if="allGame">
        <h1>游戏结束</h1>
        <p>{{gameOverMessage}}</p>
        <p>己方得分：{{selfScore}} 对方得分：{{otherScore}}</p>
        <p>请重新进入房间</p>
...
        131072: '#281d04',
  ...
  computed:{
    allGame :function(){
      return this.otherGameover&&this.gameover;
    },
    gameOverMessage :function(){
      if(this.selfScore>this.otherScore){
        //己方获胜返回
        return 'You Win!'
      }else if(this.selfScore<this.otherScore){
        //对方获胜返回
        return '胜败乃兵家常事，大侠请重新来过！' 
      }else{
        //平局
        return '大家都很强，没有分出胜负'
      }
  ...
  created() {
    bus.$on('set', ({cells, score ,gameState})=> {
      console.log(cells, score,gameState); 
      if(this.otherScore < score - 1024) {
        console.log(this.otherScore, score);
        this.punishment();
      }
      this.otherScore=score;
      this.otherCells=cells;
      this.otherGameover=gameState;
    });
    console.log(this.otherGameover);
    ...
    document.addEventListener('keyup', (event) => {
      ...
      switch (event.key.toLocaleUpperCase()) {
        ...
        case 'W':
          ...
          this.sentmes();
          if (this.gameover) {
              bus.$emit('change', {cells: this.numberCells, score: this.selfScore, gameState:this.gameover});
          }
          break;
        case 'ARROWDOWN':
        case 'S':
  ...
  methods: {
    ...
    //获取对方cell的位置（离棋盘上面的位置）
    getOtherTop(cell) {
      return `${2 + cell.y * 26}px`;
    },
    //获取对方cell的位置（离棋盘左面的位置）
    getOtherLeft(cell) {
      return `${2 + cell.x * 26}px`;
    },
    //排序棋子所需
    sortByX(a, b) {
      return a.x - b.x;
    ...
    moveLeft() {
      ...
      if(inct>1){
        this.selfScore+=inc*(inct/8)
      }
      // 凡是生成新的数字就意味着棋盘状态发生了变化
      if (canMoveLeft) {
        bus.$emit('change', {cells: this.numberCells, score: this.selfScore,gameState:this.gameover});
      }
    },
    ...
    sentmes(){
      ...
      this.$axios({
        ...
        data:this.$qs.stringify({
            ...
            type: 'simple', // score的类型，simple或者hard, String()
            score: this.selfScore // 分数，Number()
        })
      }).then(response=>{
